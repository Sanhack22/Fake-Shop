  ...
  return (
          <div key={id} className='w-72 h-96 flex flex-col relative items-center border-2 shadow-lg shadow-black/30 border-black mb-10'>
            <div className='flex flex-col items-center h-48 mb-1'>
              <h3 className='leading-6	 text-2xl max-h-14 overflow font-title3 text-center mb-5' >{title}</h3>
            ...
            <div className=''>
              <button onClick={x=>addProduct(id,title,image,category,description)} className='hover:bg-black hover:text-white duration-300 text-l w-32 font-title flex justify-center items-center bg-white text-black  text-center border-2 border-black'>Añadir</button>
            </div>
          </div>
  )
}
